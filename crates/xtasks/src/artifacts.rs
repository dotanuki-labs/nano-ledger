    ...
    match evaluate_build_environment() {
        CI => {
            ...
            for target in targets {
                ...
                let binary = format!("target/{}/release/nano-ledger", target);
                let destination = format!("{}/nano-ledger-{}", DEFAULT_ARTIFACTS_DIR, target);
...
fn compute_checksums(shell: &Shell) -> anyhow::Result<()> {
    ...
    let checksums = WalkDir::new(DEFAULT_ARTIFACTS_DIR)
        ...
        .filter(|entry| {
            ...
            path.contains("nano-ledger-x86_64") || path.contains("nano-ledger-aarch64")
        })
